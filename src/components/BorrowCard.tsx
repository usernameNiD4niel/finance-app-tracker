...
interface Props {
  ...
  receivingSourceName: string | null;
  receivingSourceIcon: string | null;
  receivingSourceColor: string | null;
  currency: string;
...
export const BorrowCard = React.memo(function BorrowCard({
  id, amount, lenderName, note, borrowDate, expectedPayDate,
  isPaid: rawIsPaid, paidDate, hasInterest: rawHasInterest, interestType, interestValue,
  receivingSourceName, receivingSourceIcon, receivingSourceColor,
  ...
  const handleLongPress = () => {
    ...
    if (!isPaid && onMarkPaid) {
      options.push({ text: 'Mark as Repaid', onPress: () => onMarkPaid(id) });
    }
    if (onEdit) {
    ...
    Alert.alert('Borrow Options', `${lenderName} - ${formatCurrency(amount, currency)}`, options);
  };
  ...
  const statusText = isPaid
    ? `Repaid ${paidDate ? formatShortDate(paidDate) : ''}`
    : isOverdue
  ...
  return (
    ...
    >
      <View style={[styles.iconContainer, { backgroundColor: (receivingSourceColor ?? theme.colors.tertiary) + '22' }]}>
        <MaterialCommunityIcons
          name={(receivingSourceIcon ?? 'cash-plus') as any}
          size={22}
          color={receivingSourceColor ?? theme.colors.tertiary}
        />
      </View>
      <View style={styles.details}>
        ...
        <Text variant="bodySmall" style={{ color: theme.colors.onSurfaceVariant }} numberOfLines={1}>
          {receivingSourceName ? `${receivingSourceName} · ` : ''}{formatShortDate(borrowDate)}
        </Text>
      ...
      <View style={styles.rightCol}>
        ...
        {!isPaid && onMarkPaid && (
          <TouchableOpacity
            style={[styles.collectBtn, { backgroundColor: theme.custom.expense + '18' }]}
            onPress={() => onMarkPaid(id)}
            activeOpacity={0.7}
          >
            <MaterialCommunityIcons name="cash-minus" size={14} color={theme.custom.expense} />
            <Text variant="labelSmall" style={{ color: theme.custom.expense, fontWeight: '700', marginLeft: 3 }}>
              Repay
            </Text>
          </TouchableOpacity>
        )}
